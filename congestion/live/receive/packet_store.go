...
type packetStore interface {
	...
	// Uses optimized single-traversal approach for btree (20% faster than duplicate check).
	...
	Insert(pkt packet.Packet) (bool, packet.Packet)
...
func (s *listPacketStore) Insert(pkt packet.Packet) (bool, packet.Packet) {
	...
	for e := s.list.Front(); e != nil; e = e.Next() {
		...
		if ph.PacketSequenceNumber == seqNum {
			return false, pkt // Duplicate - return packet for caller to release
		}
