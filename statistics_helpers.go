...
func PrintConnectionStatistics(connections []Conn, interval string, labeler ConnectionTypeLabeler) {
...
// StartStatisticsTicker starts a goroutine that periodically collects and
// prints JSON statistics for the given SRT connections. The goroutine exits
// when ctx is canceled. The connsFunc is called each tick to get the current
// set of connections (allows dynamic connection sets).
func StartStatisticsTicker(ctx context.Context, wg *sync.WaitGroup, interval time.Duration,
	connsFunc func() []Conn, labeler ConnectionTypeLabeler) {
	if interval <= 0 {
		return
	}
	if connsFunc == nil {
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				connections := connsFunc()
				PrintConnectionStatistics(connections, interval.String(), labeler)
			}
		}
	}()
}
